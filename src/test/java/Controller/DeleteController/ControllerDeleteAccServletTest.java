...
class ControllerDeleteAccServletTest {
    @Mock
    ...
    @InjectMocks
    private ControllerDeleteAccServlet servlet;
...
//проверяем, что метод handleDelete вызывается 1 раз с нужными параметрами
    void testDoDeleteSuccess() {
        ...
        verify(modelManager, times(1)).handleDelete(servlet, response, request);
    }
...
//проверяем ошибку IOException при отправке ответа response.getWriter()*******************
    void testUpdateIOExceptionGetWriter() {
        ...
        try {
            when(response.getWriter()).thenThrow(new IOException("Test ControllerDeleteServlet cannot getWriter IOException ->(All OK)"));
        } catch (IOException e) {
...
//проверяем закрытие printWriter c ошибкой закрытия
    void testUpdateIOExceptionPrintWriterClose(){
        ...
        PrintWriter printWriterMock = mock(PrintWriter.class);
        doThrow(new IOException("Test ControllerDeleteServlet IOException in close ->(All OK)")).when(printWriterMock).close();
